...
\subsection{Survey Scope, Selection Criteria, and Contributions}
...
\paragraph{Selection Scope and Representativeness.}
The reviewed set is intentionally curated rather than exhaustive: we prioritize
papers that (i) introduced a methodological turning point, (ii) became de facto
baselines in benchmarks or open-source ecosystems, or (iii) exposed a key
limitation that directly drove the next paradigm. We use citation tracing and
venue filtering to maintain representativeness across imitation learning, MARL,
cooperative perception, and LLM-based driving, while avoiding redundant
near-duplicate variants.

\subsection{Comparison with Existing Surveys}
Existing autonomous-driving surveys \citep{kiran2021deep} typically emphasize
either single-agent RL or perception-centric pipelines. In contrast, this
review explicitly centers multi-agent interaction, compares representation and
coordination mechanisms under a shared POSG lens, and includes a dedicated
analysis of recent LLM-based driving agents that are largely absent from
earlier surveys.
...
benchmarks. Section~\ref{sec:crosscut} provides cross-cutting comparison across
paradigms.
Section~\ref{sec:challenges} synthesizes open challenges and future directions.
Section~\ref{sec:conclusion} concludes.

% ---------------------------------------------------------
\section{Formal Foundations and Problem Formulation}
...
% ---------------------------------------------------------
\section{Paradigm II: Multi-Agent Interaction and Coordination}
...
% ---------------------------------------------------------
\section{Paradigm III: LLM-Based Cognitive Agents}
\label{sec:cognitive}
...
This section consolidates comparisons that span all paradigms and are not
specific to any single method family.

\subsection{Cross-Paradigm Critical Comparison}

Academic surveys usually require a \emph{large comparison table} that allows
readers to evaluate methods side by side. Table~\ref{tab:master_methods}
provides that consolidated view, spanning the major paradigms and explicitly
reporting scenario type, interaction modeling, safety mechanism, communication
assumption, and residual limitation.

\begin{table}[htbp]
\centering
\caption{Master comparison of representative methods across paradigms.}
\label{tab:master_methods}
\resizebox{\linewidth}{!}{%
\begin{tabular}{p{1.7cm} p{0.8cm} p{1.4cm} p{1.8cm} p{1.8cm} p{1.5cm} p{1.6cm} p{1.8cm} p{2.0cm}}
\toprule
\textbf{Method} & \textbf{Year} & \textbf{Paradigm} & \textbf{Core Mechanism} &
\textbf{Scenario / Benchmark} & \textbf{Safety Strategy} & \textbf{Communication Dependency} &
\textbf{Key Strength} & \textbf{Primary Limitation} \\
\midrule
DAVE-2 \citep{bojarski2016end} & 2016 & Imitation learning & End-to-end BC from raw pixels &
Lane keeping / real-road & Data-driven implicit cues & None &
Proof of end-to-end feasibility & Severe covariate shift \\

ChauffeurNet \citep{bansal2018chauffeur} & 2018 & Imitation learning & Perturbation-augmented imitation &
Urban planning logs & Auxiliary collision/off-road losses & None &
Recovery behavior learning & Limited explicit interaction modeling \\

Social LSTM \citep{alahi2016social} & 2016 & Interaction modeling &
Grid-based social pooling &
Pedestrian forecasting & Social collision avoidance prior & None &
First explicit relational prediction & Grid scaling bottleneck \\

VectorNet \citep{gao2020vectornet} & 2020 & Interaction modeling &
Vectorized scene GNN + attention &
Argoverse forecasting & Better structural priors & None &
Efficient long-range map-agent reasoning & Prediction-planning still decoupled \\

PiP \citep{song2020pip} & 2020 & Coupled prediction-planning &
Prediction conditioned on ego candidates &
nuScenes interaction planning & Joint safety-efficiency selection & None &
Mitigates ``frozen robot'' behavior & Candidate-set approximation of full game \\

RSS-HRL \citep{shalev2016safe} & 2016 & RL safety &
Learned desires + hard safety filter &
Policy learning under constraints & Rule-based hard constraints & None &
Safety-guaranteed exploration & Constraint engineering burden \\

MADDPG \citep{lowe2017multi} & 2017 & MARL &
CTDE with centralized critic &
Mixed cooperative-competitive MARL & Stable critic under non-stationarity & Train-time global state &
Strong multi-agent learning baseline & Scalability with many agents \\

V2VNet \citep{wang2020v2vnet} & 2020 & Cooperative perception &
Intermediate-feature V2V fusion GNN &
V2VSim occluded detection/prediction & Multi-view perception robustness & V2V required &
Best bandwidth-accuracy tradeoff & Sensitive to packet loss / synchronization \\

DiLu \citep{wen2023dilu} & 2023 & LLM-based cognitive agent &
Reasoning + reflection + memory retrieval &
CARLA closed-loop & Experience-informed self-correction & Optional cloud &
Continual adaptation without retraining & Reflection quality depends on LLM reliability \\

LanguageMPC \citep{sha2023languagempc} & 2023 & Neuro-symbolic LLM &
Slow-time LLM planning + fast MPC control &
Closed-loop driving tasks & MPC hard constraints + semantic planning & Optional cloud &
Balances cognition and real-time safety & End-to-end formal guarantees still open \\
\bottomrule
\end{tabular}%
}
\end{table}

Across these methods, three drivers repeatedly explain paradigm transitions:
(1) \textbf{data and compute scaling}, which enabled the shift from
DAVE-2/ChauffeurNet to VectorNet-scale encoders; (2) \textbf{benchmark
evolution}, which shifted emphasis from open-loop tests toward closed-loop
evaluation in DiLu and LanguageMPC settings; and (3) \textbf{deployment
requirements}, where interaction-heavy urban traffic motivated RSS-style safety
filters, MADDPG/CTDE stability, and V2VNet cooperative perception.
These drivers explain why the field moved from imitation-only pipelines to
interaction-aware MARL and then toward LLM-augmented cognitive architectures.
